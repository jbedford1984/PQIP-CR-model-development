...
auc_PQIP_CR <- predictions %>%
  ...
  slice_head(prop = 0.1) %>%
  ungroup() %>%
  select(POMS.overall, mean.pred)


  
## Calculate ROC for PQIP-CR model
auc_PQIP_CR <- roc(auc_PQIP_CR$POMS.overall, auc_PQIP_CR$mean.pred)

# Test difference between ROC curves between PQIP-CR, POSSUM and SORT models (recalibrated)
set.seed(23032022)
roc.test(auc_PQIP_CR, auc_SORT, method = "bootstrap")

set.seed(23032022)
roc.test(auc_PQIP_CR, auc_POSSUM, method = "bootstrap")
